...
# initialize sockets, connect, and append to list
...
for i in xrange(numTrials):
    ...
    # wait to receive data, then compare to sent
    for j in xrange(numWritesReads):
        data = socketSubset[j].recv(randomLen[j])
        start_time = time.time()
        # once the expected amount of data is received, compare the values
        while True:
